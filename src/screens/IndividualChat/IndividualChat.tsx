...
import {
  resetActiveChat,
  setActiveChat,
} from '../../redux/reducers/activeChat.reducer';
...
export const IndividualChat = () => {
  ...
  const userMobileNumber = useMemo(
    () => user.mobileNumber,
    [user.mobileNumber],
  );
  ...
  useEffect(() => {
    ...
    if (isConnectedToInternet && socket && socket.connected) {
      ...
      addUserAction(realm, SyncActionType.MESSAGE_SEEN, {
        sentAt: latestUnseenMessageSentAt,
      });
    }
  }, [
  ...
  const renderChatHeader = useCallback(
    ...
    [mobileNumber, name, originalNumber, profilePic, userMobileNumber, isUserOnline],
  );
  useEffect(() => {
  ...
  const handleClearChat = useCallback(async () => {
    try {
      ...
      if (!isConnectedToInternet) {
        addUserAction(realm, SyncActionType.CLEAR_CHAT, {
          senderMobileNumber: userMobileNumber,
          receiverMobileNumber: mobileNumber,
  ...
  const handleBlockAndUnblock = useCallback(async () => {
    ...
    try {
      if (!isAlreadyBlocked) {
        ...
        if (!isConnectedToInternet) {
          addUserAction(realm, SyncActionType.BLOCK_USER, {
            senderMobileNumber: userMobileNumber,
            receiverMobileNumber: mobileNumber,
            blockedAt: new Date(),
          });
          ...
          if (!response.ok) {
            ...
            const result = await response.json();
            showAlert(
              result.message || 'Failed to block the contact. Please try again',
              'warning',
            );
          }
        }
        ...
        unblockContactInRealm(realm, mobileNumber);
        if (!isConnectedToInternet) {
          addUserAction(realm, SyncActionType.UNBLOCK_USER, {
            senderMobileNumber: userMobileNumber,
            receiverMobileNumber: mobileNumber,
            unblockedAt: new Date(),
          });
        } else {
          const response = await unblockUserChat(
            userMobileNumber,
            mobileNumber,
          );
          if (!response.ok) {
            ...
            showAlert(
              result.message ||
                'Failed to unblock the contact. Please try again',
              'warning',
            );
          }
        }
        ...
        {showInput ? (
          ...
          <View style={styles.blockedMessageContainer}>
            <View style={styles.box}>
              <Text style={styles.blockedText}>{blockedMessageText}</Text>
            </View>
          </View>
        )}
        <ChatOptionsModal
